...
char *create_SQL_str ( opts_t *opts )
{
	...
	{
		where_len += strlen( opts->list_kvo->key[i] ) + strlen( opts->list_kvo->op[i] ) + strlen( opts->list_kvo->val[i] )
				+ ( ( i > 0 ) ? strlen( opts->bool_operators->str[i - 1] ) : 0 ) + 6;
	}
	...
	for( i = 0; i < opts->list_kvo->count; i++ )
	{
		sprintf( sql_where + strlen( sql_where ), "%s %s\"%s\" %s ", opts->list_kvo->key[i], opts->list_kvo->op[i], opts->list_kvo->val[i],
				( i < opts->bool_operators->count ) ? opts->bool_operators->str[i] : "" );
	}
